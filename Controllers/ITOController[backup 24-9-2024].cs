...
public class ITOController : Controller
{
    ...
    public ActionResult ManageAssignProgrammer(string Id, string ProductName, string AssignByLeader, string product)
    {
        if (string.IsNullOrEmpty(AssignByLeader))
        {
                                    ...
                                    string Field_21until30, string AmendmentCharges, string Activites, string Duration)
    {
        ...
        if (set == "Main")
        {
            if (!string.IsNullOrEmpty(Id) && MainProgramId != "Please Select" && TypeOfData != "Please Select" && !string.IsNullOrEmpty(MainProgramId) && !string.IsNullOrEmpty(ProgramId) && !string.IsNullOrEmpty(ProgramDesc) && !string.IsNullOrEmpty(TypeOfData) && !string.IsNullOrEmpty(StartDevOn) && !string.IsNullOrEmpty(TotalCharges))
            {
            ...
            using (SqlCommand command2 = new SqlCommand("", cn2))
            {
                ...
                while (reader.Read())
                {
                    ...
                    using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
                    {
                        ...
                        cn.Open();
                        SqlCommand command;
                        command = new SqlCommand("INSERT INTO [dbo].[SchedulerJob] (Id, Customer_Name, Frequency, JobRequest, ProductName, JobClass, JobType, SalesExecutiveBy, JobInstructionId) values (@Id, @Customer_Name, @Frequency, @JobRequest, @ProductName, @JobClass, @JobType, @SalesExecutiveBy, @JobInstructionId)", cn);
    ...
    public ActionResult AddNewLogTagNo(string Id, string Customer_Name, string ProductName, string JobClass, string Frequency, string JobType, string LogTagNo, string JobAuditTrailId, JobAuditTrailDetail get, string JobSheetNo)

    {
        ...
        using (SqlCommand command = new SqlCommand("", cn))
        {
            ...
            SqlCommand command1;
            command1 = new SqlCommand("INSERT INTO [dbo].[JobAuditTrailDetail](Id, Customer_Name, ProductName, LogTagNo,CreatedOn,Status,JobClass,JobSheetNo, JobType) values (@Id,  @Customer_Name, @ProductName, @LogTagNo,@CreatedOn,@Status,@JobClass,@JobSheetNo,@JobType)", cn);
            command1.Parameters.AddWithValue("@Id", Idx);
        ...
        return RedirectToAction("AddJAT", "ITO", new { Id = JobAuditTrailId, Customer_Name = Customer_Name, LogTagNo = LogTagNo, JobClass = JobClass, JobSheetNo = JobSheetNo });
...

    {
        ...
        using (SqlCommand command = new SqlCommand("", cn))
        {
            ...
            if (set == "search")
            {
                command.CommandText = @"SELECT DISTINCT Customer_Name, LogTagNo,JobClass,ProductName,Status,JobAuditTrailId
                                        FROM [dbo].[JobAuditTrailDetail]
                                        WHERE Customer_Name LIKE @Customer_Name ORDER BY Customer_Name";
            ...
            else
            {
                ...
                command.CommandText = @"SELECT 
                                    ...
                                    INNER JOIN
                                        JobAuditTrailDetail ON JobInstruction.JobSheetNo = JobAuditTrailDetail.JobSheetNo
                                    WHERE 
                                        JobAuditTrailDetail.JobClass='DAILY' 
                                        AND (JobAuditTrailDetail.Status='PROCESSING' OR JobAuditTrailDetail.Status='QM : Need correction from ITO')
                                    GROUP BY 
                                        JobAuditTrailDetail.LogTagNo, JobAuditTrailDetail.Customer_Name, JobAuditTrailDetail.Remarks, JobAuditTrailDetail.JobSheetNo,JobAuditTrailDetail.Id
                                    ORDER BY 
                                        JobAuditTrailDetail.LogTagNo;";
            ...
            while (reader.Read())
            {
                ...
                JobInstruction model = new JobInstruction();
                {
                    ...
                    if (reader.IsDBNull(9) == false)
                    {
                        model.JobAuditTrailId = reader.GetGuid(9);
                    }
                    if (reader.IsDBNull(10) == false)
                    {
                        model.JobSheetNo = reader.GetString(10);
                        ViewBag.JobSheetNo = reader.GetString(10);
                    }
                    if (reader.IsDBNull(11) == false)
                    {
...

    {
        ...
        using (SqlCommand command = new SqlCommand("", cn))
        {
            ...
            if (set == "search")
            {
                command.CommandText = @"SELECT DISTINCT  Customer_Name, LogTagNo,JobClass,ProductName,Status
                                        FROM [dbo].[JobAuditTrailDetail]
                                        WHERE Customer_Name LIKE @Customer_Name  ORDER BY Customer_Name ";
            ...
            else
            {
                ...
                command.CommandText = @" SELECT 
                                    ...
                                    WHERE 
                                        ...
                                        AND (JobAuditTrailDetail.Status='PROCESSING' OR JobAuditTrailDetail.Status='QM : Need correction from ITO')
                                    GROUP BY 
                                        JobAuditTrailDetail.LogTagNo, JobAuditTrailDetail.Customer_Name, JobAuditTrailDetail.Remarks, JobAuditTrailDetail.JobSheetNo
                                    ORDER BY 
                                        JobAuditTrailDetail.LogTagNo;";
    ...
    public ActionResult DeleteJobAuditTrail(string Id)
    {
        if (!string.IsNullOrEmpty(Id))
        {
            using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
            {
                cn.Open();
                SqlCommand command;
                command = new SqlCommand("DELETE [dbo].[JobAuditTrailDetail] WHERE Id=@Id", cn);
                command.Parameters.AddWithValue("@Id", Id);
                command.ExecuteNonQuery();
                cn.Close();
            }
    ...
    public ActionResult AddJAT(JobAuditTrailDetail ModelSample, DailyTracking get2, JobAuditTrailDetail get, string Id, string JobAuditTrailId, string set, string Set,
                                            ...
                                            string NotesByEngineering, string NotesByArtwork, string NotesByFinance, string NotesByDCP, string JobInstructionId,
                                            string ImageInDateOn, string ImageInTime, string RevisedInDateOn,
                                            string AccQty, string ImpQty, string PageQty, string status, string Cust_Department, string timeEndProcessIt, string revStrtDateOn, string dateApproveOn, string imageInDateOn, string revisedInDateOn, string processDate, string processEnd, string TimeTaken, string RevEndDateOn, string RevEndTimeOn)
    {
        ...
        li9.Add(new SelectListItem { Text = "Please Select", Value = "Please Select" });
        ...
        if (!string.IsNullOrEmpty(Id))
        {
            ...
            using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
            {
                ...
                using (SqlCommand command = new SqlCommand("", cn))
                {
                    ...
                    command.CommandText = @"SELECT Customer_Name, ProductName, LogTagNo,JobNameIT,ModeLog,JobId,JobAuditTrailId, Id, AccQty,ImpQty,PageQty,JobRequest,JobClass, JobSheetNo, JobType
                                                 FROM [dbo].[JobAuditTrailDetail]                                  
                                                 WHERE LogTagNo=@LogTagNo ";
                    command.Parameters.AddWithValue("@LogTagNo", LogTagNo);
                    var reader = command.ExecuteReader();
    ...
    public ActionResult SubmitPlanner(JobAuditTrailDetail get, JobAuditTrailDetail JobAuditTrailDetail, string set,
                                  ...
                                  string Status, string CreateByIT, string FileId, string JobInstructionId, string PlanDatePostOn, string ItSubmitOn, string StatusITO)
    {



        if (!string.IsNullOrEmpty(Id) && JobType == "E-BLAST")
        {
        ...
        else if (!string.IsNullOrEmpty(Id) && JobType == "DCP")
        {
        ...
        else if (!string.IsNullOrEmpty(Id) && JobClass == "DAILY")
        {
                    ...
                    if (rs4.HasRows)
                    {
                        while (rs4.Read())
                        {
                            ...
                            JobAuditTrailDetail eb = new JobAuditTrailDetail();
                            {
                                ...
                                //else
                                //{
                                //    eb.RevStrtDateOn = DateTime.MinValue;
                                //}
                                ...
                                //else
                                //{
                                //    eb.ProcessDate = DateTime.MinValue;
                                //}
                                ...
                                //else
                                //{
                                //    eb.ProcessEnd = DateTime.MinValue;
                                //}
                                ...
                                //else
                                //{
                                //    eb.DateApproveOn = DateTime.MinValue;
                                //}
                                ...
                                //else
                                //{
                                //    eb.DateProcessItOn = DateTime.MinValue;
                                //}
    ...
    public ActionResult DeleteAuditTrail(string Id, string Customer_Name, string LogTagNo, string JobClass)
    {
    ...
    public ActionResult SendBack (string JobSheetNo, string remark, string location, string from)
    {
        using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
        {
            cn.Open();
    ...
    public ActionResult SubmitCorrection(string JobSheetNo)
    {
        string Status = "";
        using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
        {
            ...
            if(Status=="QM : Need correction from MBD"||Status=="QM : Need correction from ITO")
            {
