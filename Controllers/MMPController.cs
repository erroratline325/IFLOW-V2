...
public class MMPController : Controller
{
    public ActionResult ManageMMP(string product, string set, string pageNumber, string LogTagNo)

    {
        ...
        using (SqlCommand command = new SqlCommand("", cn))
        {
            ...
            if (set == "search")
            {
                ...
                //                              WHERE JobAuditTrailDetail.Status = 'MMP' AND JobAuditTrailDetail.LogTagNo LIKE @LogTagNoSearch 
                ...
                command.CommandText = @"SELECT 
                                        ...
                                        WHERE 
                                            JobAuditTrailDetail.Status = 'MMP' AND JobAuditTrailDetail.LogTagNo LIKE @LogTagNoSearch
                                        GROUP BY 
                                            JobAuditTrailDetail.LogTagNo
                                        ORDER BY 
                                            MAX(JobAuditTrailDetail.MMPSlip) DESC;";
            ...
            else
            {
                ...
                //                            JobAuditTrailDetail.JobType,JobAuditTrailDetail.JobSheetNo, SUM(CAST(JobAuditTrailDetail.AccQty AS INT)) as AccQty, SUM(CAST(JobAuditTrailDetail.ImpQty AS INT)) as ImpQty, SUM(CAST(JobAuditTrailDetail.PageQty AS INT)) as PageQty,JobAuditTrailDetail.PrintSlip,JobAuditTrailDetail.LogTagNo,JobAuditTrailDetail.Id, JobAuditTrailDetail.MMPSlip, MAX(Hist_ProductionSlip.CreatedOn) as CreatedOn
                //                        FROM [JobAuditTrailDetail] FULL JOIN Hist_ProductionSlip ON JobAuditTrailDetail.LogTagNo = Hist_ProductionSlip.LogTagNo 
                //                        WHERE JobAuditTrailDetail.Status = 'MMP'
                //                        GROUP BY JobAuditTrailDetail.Customer_Name, JobAuditTrailDetail.ProductName, JobAuditTrailDetail.JobClass,JobAuditTrailDetail.JobType,JobAuditTrailDetail.JobSheetNo,JobAuditTrailDetail.PrintSlip,JobAuditTrailDetail.LogTagNo,JobAuditTrailDetail.Id,JobAuditTrailDetail.MMPSlip
                //                        ORDER BY (SELECT NULL)
                //                        OFFSET 0 ROWS
                //                        FETCH NEXT 100 ROWS ONLY";

                //command.CommandText = @"SELECT MAX(JobAuditTrailDetail.Customer_Name), MAX(JobAuditTrailDetail.ProductName), MAX(JobAuditTrailDetail.JobClass),
                //                                  MAX(JobAuditTrailDetail.JobType),MAX(JobAuditTrailDetail.JobSheetNo), SUM(CAST(JobAuditTrailDetail.AccQty AS INT)) as AccQty, SUM(CAST(JobAuditTrailDetail.ImpQty AS INT)) as ImpQty, SUM(CAST(JobAuditTrailDetail.PageQty AS INT)) as PageQty,MAX(JobAuditTrailDetail.PrintSlip),JobAuditTrailDetail.LogTagNo,MAX(JobAuditTrailDetail.MMPSlip),MAX(Hist_ProductionSlip.CreatedOn) as CreatedOn
                //                              FROM [JobAuditTrailDetail] FULL JOIN Hist_ProductionSlip ON JobAuditTrailDetail.LogTagNo = Hist_ProductionSlip.LogTagNo 
                //                              WHERE JobAuditTrailDetail.Status = 'MMP'     
                //                            GROUP BY JobAuditTrailDetail.LogTagNo 
                //                            ORDER BY MAX(JobAuditTrailDetail.MMPSlip) DESC";
                ...
                command.CommandText = @"SELECT 
                                        ...
                                        ON 
                                            JobAuditTrailDetail.LogTagNo = Hist_ProductionSlip.LogTagNo 
                                        WHERE 
                                            JobAuditTrailDetail.Status = 'MMP'
                                        GROUP BY 
                                            JobAuditTrailDetail.LogTagNo
                                        ORDER BY 
                                            MAX(JobAuditTrailDetail.MMPSlip) DESC;";
            ...
            while (reader.Read())
            {
                JobInstruction model = new JobInstruction();
                {
                    ...
                    {
                        model.MMPSlip = reader.GetString(10);
                    }
                    else
                    {
                        model.MMPSlip = "PENDING";
            ...
            //        if (!reader.IsDBNull(11))
            //        {
            //            model.MMPSlip = reader.GetString(11);
            //        }
            //        else
            //        {
            //            model.MMPSlip = "PENDING";
    ...
    public ActionResult CreateProdSlip(Hist_ProductionSlip get, string set, string Id, string PrintSlipNo, string Ins_StartDateOn, string Ins_StartTime, string Ins_EndDateOn, string Ins_EndTime, string Ins_Recovery,
                                       ...
                                       string NotesByIT, string NotesByProduction, string NotesByPurchasing, string NotesByEngineering, string NotesByArtwork, string NotesByFinance, string NotesByDCP, string ReturnO, string CourierO, string LogTagNo, string JobSheetNo)

    {
        ...
        using (SqlConnection cn6 = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
        {
                ...
                if (Id.ToString() == "00000000-0000-0000-0000-000000000000" && !string.IsNullOrEmpty(Id))
                {
                    ...
                    SqlCommand cmd1 = new SqlCommand(@"SELECT TOP(1) 
                                                    ...
                                                    WHERE InsInserting.LogTagNo = @LogTagNo 
                                                        AND JobAuditTrailDetail.Status = 'MMP' 
                                                    GROUP BY 
                                                        JobAuditTrailDetail.LogTagNo, 
                                                        JobAuditTrailDetail.JobSheetNo, 
            ...
            //ORIGNAL CODE, REMOVE BRACKET BEFORE DECIDING TO USE IT AGAIN, ITS USELESS ANYWAY
            {
        ...
        using (SqlConnection cn2 = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
        {
            ...
            SqlCommand cmd = new SqlCommand(@"SELECT 
                                                ...
                                                WHERE 
                                                    Hist_ProductionSlip.ProcessType = 'MMP' 
        ...
        if (set == "CreateProductionSlip")
        {
            if (string.IsNullOrEmpty(PrintSlipNo))
            {
                ...
                using (SqlConnection cn1 = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
                {
                    cn1.Open();
                    SqlCommand command1;
                    command1 = new SqlCommand("UPDATE [JobAuditTrailDetail] SET MMPSlip='CREATED' WHERE Id=@Id", cn1);
                ...
                return RedirectToAction("CreateProdSlip", "MMP", new { Id = Id, LogTagNo = LogTagNo, JobSheetNo = JobSheetNo, PrintSlipNo = PrintSlipNo }); ;
            }
        ...
        //ORIGNAL CODE, REMOVE BRACKET BEFORE DECIDING TO USE IT AGAIN, ITS USELESS ANYWAY, CLEAN THIS WHEN ITS TIME TO CLEAN CODE
        {
            //if (set == "CreateProductionSlip")
            //{
            ...
            //        return RedirectToAction("ManageMMP", "MMP", new { Id = Id });
    ...
    public ActionResult SubmitPosting(string Id, string JobInstructionId, string JobType, string set, List<JobInstruction> selectedRows, string PrintSlipNo,string LogTagNo)

    {
        ...
        using (SqlConnection cn1 = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
        {
            ...
            SqlCommand command1;
            command1 = new SqlCommand("UPDATE [JobAuditTrailDetail] SET STATUS='POSTING' WHERE LogTagNo=@Id AND Status='MMP'", cn1);
        ...
        using (SqlConnection cn1 = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
        {
            ...
            SqlCommand cmd1 = new SqlCommand(@"SELECT PrintSLipNo 
                                                    FROM Hist_ProductionSlip
                                                    WHERE LogTagNo = @LogTagNo AND ProcessType = 'MMP'", cn1);
            cmd1.Parameters.AddWithValue("@LogTagNo", LogTagNo);
            SqlDataReader rm1 = cmd1.ExecuteReader();
        ...
        //        {
        //            //new { Message = "<script>alert('Submit Failed')</script>" })
        //            RedirectToAction("ManageMMP", "MMP");
        //        }


        //        SqlCommand command1;
        //        command1 = new SqlCommand("UPDATE [JobAuditTrailDetail] SET STATUS='POSTING' WHERE Id=@Id", cn1);
        //        command1.Parameters.AddWithValue("@Id", Id);
        //        command1.ExecuteNonQuery();
        //        cn1.Close();
        //    }
    ...
    public List<string> getAIP(string LogTagNo)
    {
        ...
        using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
        {
            ...
            SqlCommand cmdAIP = new SqlCommand("SELECT SUM(CAST(AccQty AS INT)) AS AccQty, SUM(CAST(ImpQty AS INT)) AS ImpQty, SUM(CAST(PageQty AS INT)) AS PageQty FROM JobAuditTrailDetail WHERE LogTagNo = @LogTagNo AND Status='MMP'", cn);
