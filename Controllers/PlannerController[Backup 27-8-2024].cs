...

    {
        ...
        if (!string.IsNullOrEmpty(Id))
        {
            ...
            if (set == "CreatePlanSchedule")
            {
                ...
                if (!string.IsNullOrEmpty(Id) && !string.IsNullOrEmpty(StartProductionDateOn) && PlannerStatus == "PLANNER")
                {
                    ...
                    using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
                    {
                        cn.Open();
                        SqlCommand command;
                        command = new SqlCommand("INSERT INTO [Planner] ([ModifiedOn] ,[StartProductionDateOn],[PlanDatePostOn],[PlanReturn_CourierOn],[CreateByPlanner],[Id],[JobInstructionId],[StatusPlanner],[MachineInsert],[Machine],[PlanShift],[NotesByIT],[NotesByProduction],[NotesByPurchasing],[NotesByEngineering],[NotesByArtwork],[NotesByFinance],[NotesByDCP], [JobSheetNo])" +
                                      "VALUES (@ModifiedOn,@StartProductionDateOn,@PlanDatePostOn,@PlanReturn_CourierOn,@CreateByPlanner,@Id,@JobInstructionId,@StatusPlanner,@MachineInsert,@Machine,@PlanShift,@NotesByIT,@NotesByProduction,@NotesByPurchasing,@NotesByEngineering,@NotesByArtwork,@NotesByFinance,@NotesByDCP,@JobSheetNo)", cn);
                        ...
                        {
                            command.Parameters.AddWithValue("@StartProductionDateOn", null);
                        }
                    ...
                    if (NMRStatus == "Complete")
                    {
                        using (SqlConnection cn1 = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
                        {
                            cn1.Open();
                            SqlCommand command1;
                            command1 = new SqlCommand("UPDATE [JobAuditTrailDetail] SET STATUS='PRODUCTION' WHERE JobSheetNo=@JobSheetNo", cn1);
                        ...
                        using (SqlConnection cn1 = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
                        {
                            cn1.Open();
                            SqlCommand command1;
                            command1 = new SqlCommand("UPDATE [JobInstruction] SET STATUS='PRODUCTION', PrintSlip = 'PENDING' WHERE Id=@Id", cn1);
                            command1.Parameters.AddWithValue("@Id", Id);
                            command1.ExecuteNonQuery();
                            cn1.Close();
                        }
                    ...
                    else if (NMRStatus == "")
                    {
                        using (SqlConnection cn1 = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
                        {
                            cn1.Open();
                            SqlCommand command1;
                            command1 = new SqlCommand("UPDATE [JobAuditTrailDetail] SET STATUS='QME' WHERE JobSheetNo=@JobSheetNo", cn1);
                            command1.Parameters.AddWithValue("@JobSheetNo", Id);
                            command1.ExecuteNonQuery();
                            cn1.Close();
                        }
                        ...
                        using (SqlConnection cn1 = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
                        {
                            cn1.Open();
                            SqlCommand command1;
                            command1 = new SqlCommand("UPDATE [JobInstruction] SET STATUS='QME' WHERE Id=@Id", cn1);
                            command1.Parameters.AddWithValue("@Id", Id);
                            command1.ExecuteNonQuery();
                            cn1.Close();
                        }
